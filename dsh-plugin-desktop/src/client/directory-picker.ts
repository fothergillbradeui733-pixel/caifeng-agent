...
export async function requestDesktopDirectory(
  ...
  if (!response.ok) throw new Error('才丰 Agent could not open the system folder picker')
  const value: unknown = await response.json()
  if (!isResponse(value)) throw new Error('才丰 Agent received an invalid response from the system folder picker')
  return value.path
}
...
export async function requestDesktopDirectoryValidation(
  ...
  if (!response.ok) throw new Error('才丰 Agent could not validate the selected workspace')
  const value: unknown = await response.json()
  if (!isValidationResponse(value)) throw new Error('才丰 Agent received an invalid workspace validation response')
