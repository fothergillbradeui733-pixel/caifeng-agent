...
/**
 * Resolve the CaiFeng Agent GitHub Release asset URL for a platform and version.
 * Release assets are named `CaiFeng-Agent-<version>-<platform>.<ext>` and uploaded
 * by the release workflow to the `caifeng-agent` repository.
 */
export function desktopDownloadUrl(platform: DesktopDownloadPlatform, version: string): string {
  ...
  return `https://github.com/fothergillbradeui733-pixel/caifeng-agent/releases/download/v${version}/CaiFeng-Agent-${version}-${platformName}.${extension}`
}
...
export function desktopUpdateFilename(platform: DesktopDownloadPlatform, version: string): string {
  ...
  const extension = platform === 'darwin' ? 'dmg' : 'exe'
  const platformName = platform === 'darwin' ? 'mac' : 'windows'
  return `CaiFeng-Agent-${version}-${platformName}.${extension}`
}
