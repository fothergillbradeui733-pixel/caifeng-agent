...
export class ElectronDesktopRuntime implements DesktopRuntime {
  ...
  private async showRendererBootRecovery(report: Extract<RendererBootReport, { status: 'failed' }>): Promise<void> {
    ...
    const result = await dialog.showMessageBox({
      ...
      detail: `Failed plugins:\n${plugins}\n\n${error}\n\nOpen DSH Terminal to update or remove the failing third-party plugin, then restart 才丰 Agent.`,
  ...
  private async confirmUpdateDownload(version: string): Promise<boolean> {
    const result = await dialog.showMessageBox({
      type: 'info',
      title: '才丰 Agent Update Available',
      message: `才丰 Agent ${version} is available.`,
  ...
  private async showManualUpdateCheckResult(result: UpdateCheckResult | null): Promise<void> {
    if (result === null) {
      await dialog.showMessageBox({
        ...
        message: '才丰 Agent could not check for updates.',
    ...
    if (result.status === 'up-to-date') {
      await dialog.showMessageBox({
        ...
        message: 'No newer version of 才丰 Agent is available.',
    ...
    await dialog.showMessageBox({
      type: 'info',
      title: '才丰 Agent Update Available',
      message: `才丰 Agent ${result.latestVersion} is available.`,
  ...
  private async downloadAndOpenUpdate(version: string, signal: AbortSignal): Promise<void> {
    ...
    if (platform === 'darwin') {
      ...
      await dialog.showMessageBox({
        ...
        message: `才丰 Agent ${version} is ready to install.`,
        detail: 'The disk image has opened. Replace 才丰 Agent in Applications, then reopen it.',
        buttons: ['OK'],
        defaultId: 0,
        noLink: true,
      })
    ...
    const result = await dialog.showMessageBox({
      ...
      title: '才丰 Agent Update Downloaded',
      message: `才丰 Agent ${version} is ready to install.`,
      detail: 'Restart 才丰 Agent and run the installer now?',
  ...
  private async performUpdateArtifactCleanup(): Promise<void> {
    ...
    const result = await dialog.showMessageBox({
      ...
      message: zh
        ? `才丰 Agent ${artifact.version} 已安装。`
        : `才丰 Agent ${artifact.version} has been installed.`,
      detail: zh
        ? `是否删除下载的安装包以释放磁盘空间？\n\n${artifact.path}`
