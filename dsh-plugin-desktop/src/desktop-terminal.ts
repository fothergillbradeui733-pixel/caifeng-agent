...
function macWelcome(
  ...
  return [
    ...
    `printf '%s\\n' ${quoteSh(`CaiFeng Agent ${options.productVersion} terminal`)}`,
    ...
    `printf '%s\\n' ${quoteSh('Restart CaiFeng Agent after plugin changes.')}`,
...
function windowsWelcome(): string {
  ...
  return [
    ...
    `Write-Host ("CaiFeng Agent {0} terminal" -f $env:${WINDOWS_PRODUCT_VERSION})`,
    ...
    `Write-Host 'Restart CaiFeng Agent after plugin changes.'`,
    '',
  ].join('\r\n')
}
...
function windowsCmdWelcome(): string {
  ...
  return [
    ...
    `echo(CaiFeng Agent !${WINDOWS_PRODUCT_VERSION}! terminal`,
    ...
    `echo(${escapeBatchText('Restart CaiFeng Agent after plugin changes.')}`,
...
function windowsLaunchBroker(
  ...
  return [
    '@echo off',
    'setlocal EnableDelayedExpansion',
    `start "CaiFeng Agent" /D "!${WINDOWS_PROFILE_DIRECTORY}!" ${target}`,
    'exit /b %errorlevel%',
    '',
  ].join('\r\n')
}
