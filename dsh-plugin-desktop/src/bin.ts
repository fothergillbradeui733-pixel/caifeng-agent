...
Launch 才丰 Agent with the selected Web-capable profile.
...
export function defaultDesktopUserDataDirectory(
  ...
  if (platform === 'win32') {
    ...
    if (appData === undefined || appData.length === 0) {
      throw new Error('APPDATA is unavailable; cannot locate 才丰 Agent diagnostics')
    }
    return path.join(appData, '才丰 Agent')
  }
  if (platform === 'darwin') return path.join(homeDirectory, 'Library', 'Application Support', '才丰 Agent')
  const config = environment.XDG_CONFIG_HOME
  return path.join(config === undefined || config.length === 0 ? path.join(homeDirectory, '.config') : config, '才丰 Agent')
}
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      ...
      + 'Or use the packaged 才丰 Agent application.\n',
    )
    return 1
  }
