...
export class ElectronWorkspaceAdmission {
  ...
  async validateDirectory(path: string): Promise<boolean> {
    ...
    if (decision.action === 'confirm') {
      const result = await this.options.showMessageBox({
        ...
        detail: zh
          ...
          : `Disconnecting the drive while 才丰 Agent is running can break commands or plugin operations. Keep it connected.\n\n${path}`,
    ...
    await this.options.showMessageBox({
      ...
      message: zh
        ? `${decision.concern.fileSystem ?? '当前文件系统'} 不能安全用作 才丰 Agent 工作区。`
        : `${decision.concern.fileSystem ?? 'This filesystem'} cannot safely host a 才丰 Agent workspace.`,
      detail: zh
