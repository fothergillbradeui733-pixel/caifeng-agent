...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `CaiFeng-Agent-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === '才丰 Agent.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing 才丰 Agent.exe: ${portablePath}`)
  }
