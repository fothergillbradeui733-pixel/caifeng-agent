...
function macOptions(stateDir: string, spawn: DesktopTerminalSpawn): DesktopTerminalOptions {
  return {
    platform: 'darwin',
    appExecutable: "/Applications/才丰 O'Brien.app/Contents/MacOS/才丰 Agent",
...
function windowsOptions(stateDir: string, spawn: DesktopTerminalSpawn): DesktopTerminalOptions {
  return {
    platform: 'win32',
    appExecutable: 'C:\\Program Files\\才丰 100% Agent\\才丰 Agent.exe',
    dshBootstrapPath: 'C:\\Program Files\\才丰 Agent\\resources\\app.asar\\lib\\dsh-terminal-bootstrap.js',
    pnpmBinPath: 'C:\\Program Files\\才丰 Agent\\resources\\app.asar\\node_modules\\pnpm\\bin\\pnpm.mjs',
    electronVersion: '43.4.0',
    profileName: 'desktop',
    productVersion: '2.0.0',
    profileDir: "C:\\Users\\Example\\DSH O'Brien\\profiles\\desktop",
    homeDir: "C:\\Users\\Example\\DSH O'Brien",
    installRecoveryStatePath: "C:\\Users\\Example\\AppData\\Roaming\\才丰 Agent\\plugin-install-recovery\\state.json",
    stateDir,
    spawn,
    environment: {
...
describe('desktop terminal environment', () => {
  ...
  it('generates private macOS shims and opens one quoted welcome command', () => {
    ...
    expect(dshShim).toContain("'/Applications/才丰 O'\"'\"'Brien.app/Contents/MacOS/才丰 Agent'")
    ...
    expect(nodeShim).toBe([
      '#!/bin/sh',
      `ELECTRON_RUN_AS_NODE=1 exec '/Applications/才丰 O'"'"'Brien.app/Contents/MacOS/才丰 Agent' "$@"`,
      '',
    ].join('\n'))
    ...
    expect(welcome).toContain('才丰 Agent 2.0.0 terminal')
    ...
    expect(welcome).toContain('Restart 才丰 Agent after plugin changes.')
  ...
  it('generates Windows batch shims and opens PowerShell through a visible-console broker', () => {
    ...
    expect(welcome).toContain('"才丰 Agent {0} terminal" -f $env:DSH_DESKTOP_PRODUCT_VERSION')
    ...
    expect(launcher).toContain('start "才丰 Agent" /D "!DSH_DESKTOP_PROFILE_DIRECTORY!"')
  ...
  it('keeps Windows scripts ASCII and passes localized paths through the child environment', () => {
    ...
    options.appExecutable = 'C:\\程序\\才丰 Agent.exe'
    ...
    expect(harness.calls[0]?.options.env).toEqual(expect.objectContaining({
      ...
      DSH_DESKTOP_APP_EXECUTABLE: 'C:\\程序\\才丰 Agent.exe',
