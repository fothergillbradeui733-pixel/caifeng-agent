...
describe('Electron desktop runtime', () => {
  beforeEach(() => {
    ...
    updater.filename.mockImplementation((platform: string, version: string) => (
      `CaiFeng-Agent-${version}-${platform === 'darwin' ? 'mac.dmg' : 'windows.exe'}`
    ))
  ...
  it('shows native recovery when the renderer Loader reports a failed plugin', async () => {
    ...
    expect(electron.dialog.showMessageBox).toHaveBeenCalledWith(expect.objectContaining({
      ...
      message: '才丰 Agent could not load all plugins.',
      detail: expect.stringContaining('dsh-vision-router'),
      buttons: ['Open DSH Terminal', 'Restart 才丰 Agent', 'Dismiss'],
    }))
  ...
  it('uses Electron networking and confirmation-gated macOS update handoff', async () => {
    ...
    updater.download.mockResolvedValueOnce('/tmp/CaiFeng-Agent-2.1.0-mac.dmg')
    const { ElectronDesktopRuntime } = await import('../src/electron-runtime.ts')
    const runtime = new ElectronDesktopRuntime(async () => {})
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/Downloads/CaiFeng-Agent-2.1.0-mac.dmg',
    })
    ...
    expect(electron.dialog.showSaveDialog).toHaveBeenCalledWith(expect.objectContaining({
      defaultPath: join('/tmp/Downloads', 'CaiFeng-Agent-2.1.0-mac.dmg'),
      filters: [{ name: 'Disk Image', extensions: ['dmg'] }],
    }))
    expect(updater.download).toHaveBeenCalledWith({
      platform: 'darwin',
      version: '2.1.0',
      destinationPath: '/tmp/Downloads/CaiFeng-Agent-2.1.0-mac.dmg',
      request: expect.any(Function),
      signal: controller.signal,
    })
    expect(electron.shell.openPath).toHaveBeenCalledWith('/tmp/CaiFeng-Agent-2.1.0-mac.dmg')
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'darwin',
      version: '2.1.0',
      path: '/tmp/CaiFeng-Agent-2.1.0-mac.dmg',
    })
    expect(electron.dialog.showMessageBox).toHaveBeenLastCalledWith(expect.objectContaining({
      title: '才丰 Agent Update Downloaded',
      buttons: ['OK'],
    }))
  ...
  it('starts the downloaded Windows installer before requesting orderly exit', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('win32')
    updater.download.mockResolvedValueOnce('C:\\Updates\\CaiFeng-Agent-2.1.0-windows.exe')
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: 'C:\\Updates\\CaiFeng-Agent-2.1.0-windows.exe',
    })
    ...
    expect(childProcess.spawn).toHaveBeenCalledWith(
      'C:\\Updates\\CaiFeng-Agent-2.1.0-windows.exe',
      ['--updated', '--force-run'],
      {
        detached: true,
        stdio: 'ignore',
    ...
    expect(updater.record).toHaveBeenCalledWith('/tmp/dsh-desktop-user-data', {
      platform: 'win32',
      version: '2.1.0',
      path: 'C:\\Updates\\CaiFeng-Agent-2.1.0-windows.exe',
    })
  ...
  it('rejects a macOS handoff when the operating system cannot open the DMG', async () => {
    ...
    electron.dialog.showSaveDialog.mockResolvedValueOnce({
      canceled: false,
      filePath: '/tmp/CaiFeng-Agent-2.1.0-mac.dmg',
    })
