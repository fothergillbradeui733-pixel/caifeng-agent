...
function options(overrides: Partial<MacReleaseVerificationOptions> = {}) {
  ...
  const value: MacReleaseVerificationOptions = {
    ...
    listDmgs: () => ['/release/dist/才丰 Agent-2.0.0-universal.dmg'],
...
describe('macOS release artifact verification', () => {
  it('mounts one DMG and verifies signature, Gatekeeper, and the stapled ticket', () => {
    ...
    const appPath = join('/private/tmp/dsh-desktop-dmg-test', '才丰 Agent.app')

    expect(verifyMacRelease(harness.value)).toEqual({
      appPath,
      dmgPath: '/release/dist/才丰 Agent-2.0.0-universal.dmg',
    })
    ...
    expect(harness.calls).toEqual([
      {
        command: 'hdiutil',
        args: [
          'attach', '/release/dist/才丰 Agent-2.0.0-universal.dmg',
          '-mountpoint', '/private/tmp/dsh-desktop-dmg-test', '-nobrowse', '-readonly',
        ],
      },
      {
        command: 'lipo',
        args: [join(appPath, 'Contents', 'MacOS', '才丰 Agent'), '-verify_arch', 'x86_64'],
      },
      {
        command: 'lipo',
        args: [join(appPath, 'Contents', 'MacOS', '才丰 Agent'), '-verify_arch', 'arm64'],
      },
