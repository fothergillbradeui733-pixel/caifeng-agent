...
describe('packaged dsh bootstrap', () => {
  ...
  it('clears Node mode before loading the fixed packaged CLI entry', async () => {
    ...
    const argv = ['/Applications/才丰 Agent', '/app.asar/lib/desktop-cli.js', '--dump-config']
    const load = vi.fn(async (url: string) => {
      expect(environment).toEqual({ KEEP: 'value' })
      expect(argv).toEqual([
        '/Applications/才丰 Agent',
  ...
  it('uses the physical unpacked dependency tree only inside an Electron package', () => {
    expect(unpackedAsarPath('/Applications/才丰 Agent.app/Contents/Resources/app.asar/node_modules/pkg'))
      .toBe('/Applications/才丰 Agent.app/Contents/Resources/app.asar.unpacked/node_modules/pkg')
    expect(unpackedAsarPath('C:\\Program Files\\才丰 Agent\\resources\\app.asar\\node_modules\\pkg'))
      .toBe('C:\\Program Files\\才丰 Agent\\resources\\app.asar.unpacked\\node_modules\\pkg')
    expect(unpackedAsarPath('/Applications/才丰 Agent.app/Contents/Resources/app.asar/package.json'))
      .toBe('/Applications/才丰 Agent.app/Contents/Resources/app.asar.unpacked/package.json')
