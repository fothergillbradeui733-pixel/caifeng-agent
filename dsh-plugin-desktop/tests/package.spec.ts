...
describe('published package surface', () => {
  ...
  it('fixes the installed application identity', () => {
    ...
    expect(manifest.build?.win?.artifactName).toBe('CaiFeng-Agent-${version}-${arch}-Portable.${ext}')
    expect(manifest.build?.nsis).toEqual({
      ...
      shortcutName: '才丰 Agent',
      useZip: true,
      artifactName: 'CaiFeng-Agent-${version}-${arch}-Setup.${ext}',
    })
